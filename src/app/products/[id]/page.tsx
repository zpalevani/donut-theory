...
// Required for static export - tells Next.js which product pages to generate at build time
export function generateStaticParams() {
  return donuts.map((donut) => ({
    id: donut.id.toString(),
  }));
}
...
export default async function ProductPage({ params }: { params: Promise<{ id: string }> | { id: string } }) {
  ...
  const donut = donuts.find((d) => d.id === parseInt(resolvedParams.id));
